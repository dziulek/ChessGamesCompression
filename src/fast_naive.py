...
from compressors import REV_SCORE_MAP, SCORE_MAP
from utils import to_binary, read_binary, extract_move_idx, sort_moves, move_from_code
from utils import processLine, get_script_path
...
def decode_naive(buff: io.TextIOWrapper, batch_s: int) -> List[str]:

    MOVE_REGEX = r'O-O-O|O-O|(([QKRBN])?([a-h])?(x)?[a-h][1-8](([#+])|(=[QRBN]([+#])?))?)|1/2-1/2|1-0|0-1'
    ...
    while byte_it < len(enc_data):
        ...
        _bin = int.from_bytes(enc_data[byte_it : byte_it + 4], 'big')
        byte_it += 4
